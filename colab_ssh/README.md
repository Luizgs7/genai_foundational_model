...
| `Permission denied (publickey)` | Sua chave ainda não está no `authorized_keys` da sessão atual (cada reinício do Colab zera isso) | Confirma que sua chave está em `colab_ssh/authorized_keys` no repo, e que a célula do passo 3 já rodou **depois** que sua chave foi adicionada. |
